...
data = {
    ...
    'weather_score': np.random.uniform(1, 10, n_samples),
    'shipment_weight_kg': np.random.uniform(5, 2000, n_samples),
    'num_stops': np.random.randint(0, 8, n_samples),
    'driver_experience_yrs': np.random.uniform(0.5, 20, n_samples),
    'vehicle_type': np.random.choice(['Van', 'Box Truck', 'Refrigerated Truck', 'LTL'], n_samples),
    'time_of_day': np.random.choice(['Morning', 'Afternoon', 'Evening', 'Night'], n_samples),
    'day_of_week': np.random.choice(['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], n_samples),
    'route_type': np.random.choice(['Urban', 'Highway', 'Mixed', 'Rural'], n_samples),
}
